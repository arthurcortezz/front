...
import { UnityTypePaginatedInterface } from '../../unity-types.types';

@Component({
  selector: 'unity-types-list',
  templateUrl: './unity-types-list.component.html',
  encapsulation: ViewEncapsulation.None,
})
export class UnityTypesComponent implements OnInit, OnDestroy {
  public data: UnityTypePaginatedInterface = null;
  ...
  getAll(): void {
    this.service
      ...
      .subscribe((res: UnityTypePaginatedInterface) => {
        this.data = res;
      });
  }
  ...
  handleAction(data: AcsHeaderActionInterface): void {
    switch (data.action) {
      case 'form':
        if (!data.id) {
          this.router.navigateByUrl('unidades/criar');
        } else {
          this.router.navigateByUrl(`unidades/editar/${data.id}`);
        }
        break;
      case 'delete':
