...
import { AcsMockApiService } from '@acs/mock-api/mock-api.service';
...
export class AcsMockApiInterceptor implements HttpInterceptor {
  constructor(
    @Inject(ACS_MOCK_API_DEFAULT_DELAY)
    private readonly defaultDelay: number,

    private readonly authService: AuthService,

    private readonly service: AcsMockApiService
  ) {}
  ...
  intercept(
    ...
    const { handler, urlParams } = this.service.findHandler(
      request.method.toUpperCase(),
      request.url.replace('@acs-api', 'api')
    );
